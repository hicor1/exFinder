...
// [KREAM_목록정보] 페이지 번호와 페이지당 항목 수를 인자로 받습니다. ( 1과 10은 기본값!!, 함수 호출할때 변경 가능)
...
// [KREAM_상세정보] 페이지 번호와 페이지당 항목 수를 인자로 받습니다. ( 1과 10은 기본값!!, 함수 호출할때 변경 가능)
export const getKREAM_detail = async ({ productId = "L47298400", option = "245" }) => {
  
  const SQL_QUERY = `
    SELECT *
    FROM kream_matched
    WHERE 품번_1 = '${productId}' AND 옵션_1 = '${option}'
  `; // 페이지당 항목 수와 시작점을 적용합니다.
